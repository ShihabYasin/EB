   ...
   ********************/
        ...
        getNumber: function(type) {
            if (window.intlTelInputUtils) {
                var number = intlTelInputUtils.formatNumberByType(this.telInput.val(), this.selectedCountryData.iso2, type);
                //alert("before changing - " + number[0]);
                if (number[0] == "+")
                    number = number.substring(1, number.length);
                //alert("funciton changed - "+ number);
                return number;
            }
            return "";
        },
